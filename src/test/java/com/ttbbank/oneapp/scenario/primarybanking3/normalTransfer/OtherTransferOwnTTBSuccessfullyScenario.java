...
public class OtherTransferOwnTTBSuccessfullyScenario extends PrimaryBanking3Base {
    String fromAccountNumber;
    String toAccountNumber;

    @Test(groups = "Transfer other own ttb success", priority = 1)
    ...
    @Test(groups = "Transfer other own ttb success", priority = 2)
    ...
    @Test(groups = "Transfer other own ttb success", priority = 3)
    ...
    @Test(groups = "Transfer other own ttb success", priority = 4)
    public void testDataAssertEqualsEnterDetailsOnReviewScreenAndTapOnConfirm() throws IOException {
        ...
        evaluateAssertEqual(
                context.findElement(OneAppElementKeys.PB3_OTHER_TRANSFER_OWN_ACCOUNT_NUMBER).getText(),
                toAccountNumber,
                "Validate to account number is displayed correctly (the same as entered in enter detail before)"
        );

        evaluateAssertEqual(
                context.findElement(OneAppElementKeys.PB3_OTHER_TRANSFER_OWN_ACCOUNT_NAME).getText(),
                "basic account",
                "Validate to account nickname is displayed correctly (the same as display in enter detail before)"
        );
    ...
    @Test(groups = "Transfer other own ttb success", priority = 5)
