...
public class APITestPaymentList {
    ...
    private void getPaymentListSuccess() {
        ...
        String body = bodyObj.getPaymentListSuccess().toJSONString();
        ...
        String response = given().relaxedHTTPSValidation().auth().oauth2(token).log().all()
                ...
                .body(body).when()
                .post("/apis/hpservice/payment-list").then().assertThat().extract().response().asString();
    ...
    private void getPaymentListBadRQ() {
        token = AutoloanUtility.genToken("eyJzZXJ2aWNlVHlwZSI6IkFMMDEwMCIsInRpbWVTdGFtcCI6MTYxNTE4ODAzNjE0NSwiYWNjb3VudE51bWJlciI6IjI2NjE1NTgxMCIsIm5pY2tOYW1lIjoiSGlyZVB1cmNoYXNlLTU4MTAifQ==");
        GetJsonData bodyObj = new GetJsonData();
        String body = bodyObj.getPaymentListBadRQ().toJSONString();
    ...
    @Test(groups = "API Test Payment List")
    public void apiTestPaymentList() {
