...
public class TC_1386_AT_201 extends Base {
	...
	public void ADTest001() throws IOException {
		...
		ws.waitVisible(ws.saLbTitle);
		
		ws.assertContain(ws.fX(ws.saLbTitle).getText(), map("Title"));
		ws.assertText(ws.fX(ws.saLbDesc1).getText(), map("Desc1"));
		ws.assertText(ws.fX(ws.saLbDesc2).getText(), map("Desc2"));
		ws.assertText(ws.fX(ws.saBtnNext).getText(), map("Next"));
