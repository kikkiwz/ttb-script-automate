...
public class API_TC06_IncompleteTranMobile extends Base {
	...
	@Test(groups = { "android", "PB1" }, dataProvider = "getExcelData", description = "IncompleteTransaction mobile")
...
//Transfer and Validation -------------------------------------------------------------------
		toAccountNo = "0".concat(String.valueOf(Long.parseLong(aa1) - 1));
		System.out.println("/////////to wrong mobile no///////////-  " + "\n" + toAccountNo);
		...
		response = given().relaxedHTTPSValidation().log().all().header("Content-Type", "application/json")
				.header("X-PROXY-TYPE", "2").header("Channel", "mb").header("X-Correlation-ID", XCorrelationI)
				.header("X-CRMID", crmId).auth().oauth2(access_tokenV).body(hm).when().post(promptApi).then()
