...
public class APITestCarTaxSign {
    private String token;

    private void CarTaxSignOnlinePaymentSuccess() {
        ...
        String body = bodyOjb.getCarTaxSignOnlinePaymentSuccess().toJSONString();
        ...
        String response = given().relaxedHTTPSValidation().auth().oauth2(token).log().all()
                ...
                .body(body).when()
                .post("/apis/hpservice/service-request/car-tax-sign").then().assertThat().extract().response().asString();
    ...
    private void CarTaxSignOnlinePaymentBadRQ() {
        token = AutoloanUtility.genToken("eyJzZXJ2aWNlVHlwZSI6IkFMMDIwMSIsInRpbWVTdGFtcCI6MTYxNzA4Nzg2MjY0NSwiQ0lEIjoiMzEzOTkwMDAyMzg0MCIsImxhbmd1YWdlIjoiZW4ifQ==");
        GetJsonData bodyOjb = new GetJsonData();
        String body = bodyOjb.getCarTaxSignOnlinePaymentBadRQ().toJSONString();
        RestAssured.baseURI = "https://oneapp-sit1.tau2904.com";
        String response = given().relaxedHTTPSValidation().auth().oauth2(token).log().all()
                .header("X-Correlation-Id", "4e953db0-f5e7-4cf9-b44f-2819ee8e7866")
                .header("Content-Type", "application/json")
    ...
    @Test(groups = "API Test Car Tax Sign")
    public void apiTestCarTaxSign() {
        CarTaxSignOnlinePaymentSuccess();
        CarTaxSignOnlinePaymentBadRQ();
    }
