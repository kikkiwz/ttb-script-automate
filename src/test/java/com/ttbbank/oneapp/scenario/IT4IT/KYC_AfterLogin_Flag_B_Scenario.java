...
public class KYC_AfterLogin_Flag_B_Scenario extends BaseScenario {
	...
	@Test(groups = "KYC verify after login with flag B")
	public void KYC_Review_AfterLogin_Flag_B() throws InterruptedException, IOException {
		...
		try {
			...
			String Image = "images/IT4IT/KYCB/logged_in_"+new SimpleDateFormat("HHmmss").format(new Date())+".png";
		...
		try {
			...
			String Image = "images/IT4IT/KYCB/Register_"+new SimpleDateFormat("HHmmss").format(new Date())+".png";
			takeScreenshot(Image);
		...
		try {
			...
			String Image = "images/IT4IT/KYCB/App_Tutorial"+new SimpleDateFormat("HHmmss").format(new Date())+".png";
			takeScreenshot(Image);
...
try {
		...
		context.findElement(OneAppElementKeys.SETTING_TAB).click();
		enterPin();
		context.waitUntilVisible(OneAppElementKeys.KYC_PAGE_NEXT);
		String Image = "images/IT4IT/KYCB/KYC_B_PAGE"+new SimpleDateFormat("HHmmss").format(new Date())+".png";
		takeScreenshot(Image);
		Scenario1.createNode(Given.class, "After Clicking on setting tab KYC page not loaded").addScreenCaptureFromPath(Image).pass("PASS");			
}
catch(Throwable t) {
	String Image = "images/IT4IT/KYCB/KYC_B_PAGE"+new SimpleDateFormat("HHmmss").format(new Date())+".png";
	takeScreenshot(Image);
	Scenario1.createNode(Given.class, "After Clicking on setting tab KYC page not loaded").addScreenCaptureFromPath(Image).fail(t);			
		...
		try {
		context.findElement(OneAppElementKeys.KYC_PAGE_NEXT).click();
		confirmPin();
		Thread.sleep(2000);
		Activity_log("001100000000000000000012189002","100000100","Verify Current PIN","Success"," KYC_VERIFY_PIN_STATUS",0,Scenario1);
		...
		String Image = "images/IT4IT/KYCB/KYC_B_"+new SimpleDateFormat("HHmmss").format(new Date())+".png";
		takeScreenshot(Image);
		Scenario1.createNode(Given.class, "Personal Information page inside KYC page successfully loaded for CRM ID set to flag B").addScreenCaptureFromPath(Image).pass("PASS");			

	}
catch(Throwable t) {
	String Image = "images/IT4IT/KYCB/KYC_B_"+new SimpleDateFormat("HHmmss").format(new Date())+".png";
	takeScreenshot(Image);
	Scenario1.createNode(Given.class, "Unable to load Personal Information page for CRM ID set to flag B").addScreenCaptureFromPath(Image).fail(t);			
}
