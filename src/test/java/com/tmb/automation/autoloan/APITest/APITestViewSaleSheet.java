...
public class APITestViewSaleSheet {
    ...
    private void getViewSaleSheetSuccess() {
        ...
        String body = bodyObj.getViewSaleSheetSuccess().toJSONString();
        ...
        String response = given().relaxedHTTPSValidation().auth().oauth2(token).log().all()
                ...
                .body(body).when()
                .post("/apis/hpservice/sale-sheet").then().assertThat().extract().response().asString();
    ...
    private void getViewSaleSheetFailed() {
        token = AutoloanUtility.genToken("eyJzZXJ2aWNlVHlwZSI6IkFMMDEwMCIsInRpbWVTdGFtcCI6MTYxNTE4ODAzNjE0NSwiYWNjb3VudE51bWJlciI6IjI2NjE1NTgxMCIsIm5pY2tOYW1lIjoiSGlyZVB1cmNoYXNlLTU4MTAifQ==");
        GetJsonData bodyObj = new GetJsonData();
        String body = bodyObj.getViewSaleSheetFailed().toJSONString();
        RestAssured.baseURI = "https://oneapp-sit1.tau2904.com";
        String response = given().relaxedHTTPSValidation().auth().oauth2(token).log().all()
                .header("X-Correlation-ID", "4e953db0-f5e7-4cf9-b44f-2819ee8e7866")
                .header("Content-Type", "application/json")
        ...
        String StatusCode = jsonPath.getString("status.code");
        String StatusMSG = jsonPath.getString("status.message");
        Assert.assertEquals(StatusCode, "0009");
        Assert.assertEquals(StatusMSG, "DATA NOT FOUND");
        saveLog(response);
    }

    @Test(groups = "API Test View sale sheet")
    public void apiViewSaleSheet() {
        getViewSaleSheetSuccess();
        getViewSaleSheetFailed();
    }
}
