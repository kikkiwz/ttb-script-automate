...
public class API_TC03_CancelRequestWrongRef extends Base {
	...
	@Test(groups = { "android", "PB1" }, description = "CancelRequest Wrong Reference ID")
...
// Verify Cancel Request ReferenceID --------------------------------------------------------
		...
		hmC3.put("reference_id", "xxxxx");
		System.out.println("/////////bodyC3///////////-  " + "\n" + hmC3);

		response = given().relaxedHTTPSValidation().log().all().header("Content-Type", "application/json")
				.header("X-Correlation-ID", XCorrelation).auth().none().body(hmC3).when()
				.post(CancelRequest).then().assertThat().statusCode(400).extract().response();
		...
		String CodeExpectedV11 = "ekyc_close_0001";
		...
		String messageexpectedV1 = "the reference id is not found or service is inconvenient.";
...
//		Assert.assertEquals(messageV1S, messageexpectedV1S);
		...
		String messageexpectedV1D = "Unable to proceed. For more information please call 1428.";
		System.out.println("/////////AssertmessageV1D///////-  " + "\n" + messageV1D);
//		Assert.assertEquals(messageV1D, messageexpectedV1D);

		System.out.println("///////////Verify Cancel Request Wrong ReferenceID Passed/////////////// ");	
	}
