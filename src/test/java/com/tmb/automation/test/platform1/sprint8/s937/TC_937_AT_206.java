...
public class TC_937_AT_206 extends Base {
	
	@Parameters({"DeviceProfile" })
	@BeforeMethod(groups= {"android", "plaform1", "Sprint8", "935",})
	public void BeforeTest(String deviceProfile) {
		Helper.CONFIG_PROFILE = deviceProfile;
		Helper.resetEmail("001100000000000000000012027155", true);
...
//		hs.waitVisible(homePage.swTxtEmail);
		hs.assertText(hs.fX(ver.swLbVerifyEmail_Title).getText(), map("Verify_Title_TH"));
		hs.assertText(hs.fX(ver.swLbVerifyEmail_Description).getText(), map("Verify_Description_TH"));
		hs.assertTrue(hs.isElementPresent(ver.swTxtVerifyEmail_Email));
		hs.assertText(hs.fX(ver.swBtnVerifyEmail_VerifyEmail).getText(), map("Verify_Button_TH"));
		...
		for (int i = 0; i < testData.length-1;i++) {
			...
			sleep();
			ver.setText2(hs.fX(ver.swTxtVerifyEmail_Email), testData[i]);
			log("Click Verify button");
			hs.click(hs.fX(ver.swBtnVerifyEmail_VerifyEmail));
//			Helper.writeLog(appiumDriver.getPageSource());
			hs.assertText(hs.fX(ver.swLbVerifyEmail_EmailErrorMsg).getText(), map("Email_ErrorMsg_TH"));
			sleep(1);
		}
		...
		hs.fX(ver.swTxtVerifyEmail_Email).clear();
		sleep();
		ver.setText2(hs.fX(ver.swTxtVerifyEmail_Email), testData[testData.length-1]);
		log("Click Verify button");
		hs.click(hs.fX(ver.swBtnVerifyEmail_VerifyEmail));
...
//		//verify screen
		hs.assertText(ver.fX(ver.swBtnOTPResendCode).getText(), map("Resend_Button_TH"));
...
//		//Verify valid PIN success
		...
		hs.assertText(Otp_TutanotaPage.contentEmail_TH, mapData.get("Content_Email_TH"));
		hs.setTextOTP(hs.fX(ver.swTxtVerifyEmail_Otp_Pin1), otpNumber.split("")[0]);
		hs.setTextOTP(hs.fX(ver.swTxtVerifyEmail_Otp_Pin2), otpNumber.split("")[1]);
		hs.setTextOTP(hs.fX(ver.swTxtVerifyEmail_Otp_Pin3), otpNumber.split("")[2]);
		hs.setTextOTP(hs.fX(ver.swTxtVerifyEmail_Otp_Pin4), otpNumber.split("")[3]);
...
//		//Verify success verify email page
		hs.assertText(hs.fX(ver.swLbOTPSuccessTitle).getText(), map("OTP_Success_Title_TH"));
		hs.assertText(hs.fX(ver.swLbOTPSuccessMsg).getText(), map("OTP_Success_Msg_TH" ));
		hs.assertText(hs.fX(ver.swBtnOTPGoToHome).getText(), map("OTP_Success_Gotohome_TH"));
//		
...
//		
		hs.assertText(hs.fX(hs.swLbHS_ChangeEmail).getText(), map("ChangeEmail_Button_TH"));
		hs.assertText(hs.fX(hs.swLbHS_ChangeEmail_Content).getText(), map("ChangeEmail_Content_TH"));
