...
public class PromptpayMobileSuccessfullyScenario extends PrimaryBanking3Base {
    String fromAccountNumber;
    String toAccountNumber;

    @Test(groups = "Transfer Promptpay mobile success", priority = 1)
    ...
    @Test(groups = "Transfer Promptpay mobile success", priority = 2)
    ...
    @Test(groups = "Transfer Promptpay mobile success", priority = 3)
    public void testFilledPromptpayTransferEnterDetailAndNextToReviewScreenSuccess() throws IOException {
        ...
        context.findElement(OneAppElementKeys.PB3_ENTER_DETAIL_PROMPTPAY_CATEGORY).click();
...
//
    @Test(groups = "Transfer Promptpay mobile success", priority = 4)
    public void testDataAssertEqualsEnterDetailsOnReviewScreenAndTapOnConfirm() throws IOException {
        ...
        evaluateAssertEqual(
                context.findElement(OneAppElementKeys.PB3_PROMPTPAY_MOBILE_REVIEW_TO_ACCOUNT_NUMBER).getText(),
                toAccountNumber,
                "Validate to account number is displayed correctly (the same as entered in enter detail before)"
        );

        evaluateAssertEqual(
                context.findElement(OneAppElementKeys.PB3_PROMPTPAY_MOBILE_REVIEW_AMOUNT).getText(),
                "99.00",
    ...
    @Test(groups = "Transfer Promptpay mobile success", priority = 5)
    public void testDataAssertEqualsReviewScreenAndTapOnBackToHomeSuccess() throws IOException {
        setScenario("5. Customer is on complete screen, validate information is correctly and done button go to the transfer landing screen");
        TtbTestContext context = getTtbTestContext();
        ...
        evaluateAssertEqual(
                context.findElement(OneAppElementKeys.PB3_PROMPTPAY_MOBILE_COMPLETE_AMOUNT_VALUE).getText(),
                "99.00",
        ...
        createScreenShot("Show landing screen", true);
    }
}
